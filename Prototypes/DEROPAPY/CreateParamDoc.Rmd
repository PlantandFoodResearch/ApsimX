...
#head(work_data)
```
# create graphs with static parameters values
```{r, fig.height= 15}

save_path <- "C:\\github\\ApsimX\\Prototypes\\DEROPAPY\\outputs\\ParamGraphs"
...
df_temp <- work_data %>%
  mutate(Category = if_else(str_detect(Category, "Carbon"), "Carbon", Category))

cats <- unique(df_temp$Category)

for(i in 1:length(cats)) {
  
  # filter  
  thisCat_data <- df_temp %>%
 # filter(ParamName == select_param) %>%
  ...
  mutate(CropName=factor(CropName, levels=crop_select)) %>%
  ggplot() +
  geom_bar(stat="identity", aes(x=CropName,y=ParamValue, fill = CropType)) +
  facet_wrap(.~ParamNameFull, scales="free", ncol=3) +
  coord_flip() +
  ...
  ggsave(filename = file.path(save_path,paste0(cats[i],".jpg")), dpi=300 )
...
```
# Create graphs of dynamic parameters
```{r}
...
p <- df6 %>%
filter(str_detect(ParamNameFull, c("TT"))) %>%
mutate(Category = if_else(Category == "CarbonSupply", "Stress", Category)) %>%
...
ggplot(aes(x = `Axis_X`, y = `Axis_Y`)) +
  ...
  geom_line(aes(colour=CropName, linetype= CropType ), 
            position=position_jitter(w=0.1, h=0), size=2) +
  facet_wrap(~ ParamNameFull, scales = "free", ncol=2) +
  labs(title = "Parameters for different crops",
       x = "X-axes",
       y = "Y-axes") +
  theme_minimal() +
  theme(text=element_text(size=20)) +
  ylab("Thermal-time accumulation (oCd)") +
  xlab("Average air temperature (oC)")+
  theme(legend.title = element_blank())

    
  print(p)
  
  ggsave(filename = file.path(save_path,paste0(paste0("TT_",cats[i]),".jpg")), dpi=300 )

}
```


# Other pheno stress graphs
```{r, fig.height=20}

cats <- unique(df6$Category) 

cats <- c("Phenology", "Stress")

for(i in 1:length(cats)) {
      
p <- df6 %>%
filter(!str_detect(ParamNameFull, c("TT|Dorm"))) %>%
mutate(Category = if_else(Category == "CarbonSupply", "Stress", Category)) %>%
filter(Category== cats[i]) %>%
ggplot(aes(x = `Axis_X`, y = `Axis_Y`)) +
  geom_point(aes(colour = CropName), shape=21, size=5,
  ...
  ggsave(filename = file.path(save_path,paste0(paste0("Array_",cats[i]),".jpg")), dpi=300 )
...
p <- df6 %>%
filter(!str_detect(ParamNameFull, c("TT|Dorm"))) %>%
filter(str_detect(CropName, c("Grape|Wheat|OSR")))  %>%
mutate(Category = if_else(Category == "CarbonSupply", "Stress", Category)) %>%
filter(Category== cats[i]) %>%
ggplot(aes(x = `Axis_X`, y = `Axis_Y`)) +
  geom_point(aes(colour = CropName), shape=21, size=5,
  ...
  geom_line(aes(colour=CropName, linetype= CropType ), 
            position=position_jitter(w=0.1, h=0), size=2) +
  facet_wrap(~ CropName, scales = "free", ncol=2) +
  labs(
       x = "X-axes",
       y = "Y-axes") +
  theme_minimal() +
  theme(text=element_text(size=30)) +
  ylab("Accumulation of chilling (fractional)") +
  ...
  ggsave(filename = file.path(save_path,paste0(paste0("Chill_",cats[i]),".jpg")), dpi=100 )
