...
# Plot absolute diferences as GRAPHS --- 2 graphs per ISLAND
```{r, fig.width=15}
...
p <- diff_abs_plot %>% 
 ...
 # facet_wrap(REGC2015_N~VarName, scales = "free") +
  ...
  #labs(title = paste0(thisCrop[i], " - Yield changes across regions\n")) +
  labs(x = NULL) +
  labs(caption = paste0("Figure A",figNumber," Yield differences under climate change Representative Concentration Pathways (RCP)
  for ",thisCrop[i]," across New Zealand. Region-subset ",j," of 4. Selected regions ordered by latitudinal range.")) +
  theme(
...
# Plot 4 SELECTED REGIONS panels per crop graph
```{r, fig.width=15}
...
#for(j in 1:length(region_groups)) {
  ...
  selected_regions <- region_by_crop_groups[[thisCrop[i]]]  
    
  figNumber <- figNumber + 1
  
  figTitle<-paste0(thisCrop[i],"_selected_regions")
  
  print(figTitle)
  
p <- diff_abs_plot %>% 
  inner_join(nz_lat_regions, by="REGC2015_N") %>%
  filter(REGC2015_N %in% selected_regions) %>%
  filter(VarName != "Mid-century") %>%
  mutate(REGC2015_N = factor(REGC2015_N, 
                             levels =(unique(REGC2015_N[order(desc(Latitude))]))))%>%
  mutate(Irrigation = factor(Irrigation, levels=c("Rainfed","Irrigated"))) %>%
    filter(Crop == thisCrop[i]) %>%
  ...
  scale_fill_brewer (palette = 5, direction =-1) +
  facet_wrap(REGC2015_N~VarName, scales = "free") +
  # facet_grid(RegionLat~VarName) + # , scales="free"
  geom_hline(yintercept = 0, linetype = "dashed", color = "darkgrey", size = 1) +
  theme(text = element_text(size = fontSize2),
        legend.position = "top",
        strip.text = element_text(size = fontSize2*0.9),
...
# Save the plot as a JPEG file with 300 DPI
ggsave(filename = file.path(thisGraphFolder,"SelectedRegionsPerCrop",paste0(figTitle,".jpg")),
       plot = p,
     #  width = 8,    # Width in inches
     #  height = 4,   # Height in inches
       dpi = 300)
