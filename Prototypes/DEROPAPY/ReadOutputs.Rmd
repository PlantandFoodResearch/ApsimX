...
```{r, warning=FALSE}
# Create a common baseline for each pixel (median 30-year)
yield_base <- NULL
yield_base <- data_work %>%
  ...
  ungroup() %>%
  #tidyr::crossing(RCP = c("RCP4.5", "RCP6.0", "RCP8.5")) %>%
  # mutate(RCP = "RCPpast", RCP=factor(RCP)) %>%
  #mutate(TimeSlice = "Baseline") %>%
  as.data.frame()
...
yield_norm <- data_work %>%
  ...
  ungroup() %>%
  mutate(YieldNorm = round((ProductFresh_kgPerHa/BaseFreshYield)*100,0)-100) 
...
unique(yield_norm_summary$REGC2015_N)

df_simple_regions <- yield_norm_summary %>%
  filter(REGC2015_N %in% c("Northland","Hawke's Bay","Waikato","Canterbury" ,"Southland")) %>%
 # filter(REGC2015_N %in% c("Northland" ,"Southland")) %>%
  mutate(REGC2015_N=factor(REGC2015_N, 
                           levels=c("Northland","Hawke's Bay","Waikato","Canterbury" ,"Southland")))
```


```{r}
df_simple_regions %>%
    ggplot(aes(x=TimeSliceYear, y=Median, colour=RCP)) +
  ...
  geom_hline(yintercept = 0, linetype = 2, color = "darkgrey", size = 2)+
    ...
    # geom_errorbar(aes(ymin = Median - SD/2, ymax = Median + SD/2, colour=RCP), 
    #               width = 1) +
